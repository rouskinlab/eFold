...
class Loader:
    def __init__(
        ...
        self.path = path + ".pkl" if path is not None and not path.endswith(".pkl") else path
        makedirs(dirname(self.get_path()), exist_ok=True)

    def get_path(self, extension='.pkl'):
        return join(
            "models", wandb.run.name + extension if wandb.run is not None else "default_name" + extension
