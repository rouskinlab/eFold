...
def convert_bp_list_to_dotbracket(bp_list: list[tuple[int, int]], seq_len: int) -> str:
    ...
    chars_set = [("(", ")"), ("(", ")"), ("[", "]"), ("{", "}"), ("<", ">")]
    alphabet = [
        (chr(lower), chr(upper)) for upper, lower in zip(list(range(65, 91)), list(range(97, 123)))
    ]
    ...
    for group, chars in zip(groups, chars_set):
        for bp in group:
            db = db[: bp[0]] + chars[0] + db[bp[0] + 1 : bp[1]] + chars[1] + db[bp[1] + 1 :]
    return db


def _group_into_non_conflicting_bp(bp_list: list[tuple[int, int]]) -> list[list[tuple[int, int]]]:
    ...
    while non_redudant_bp_list != []:
        ...
        max_group = [bp for bp in non_redudant_bp_list if bp not in current_bp_conflicts]
        to_remove = []
        ...
        conflict_list = [
            conflict
            for conflict in conflict_list
            if conflict[0] not in group and conflict[1] not in group
        ]
    return groups


def _get_non_redudant_bp_list(conflict_list: list) -> list:
