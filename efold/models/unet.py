...
class U_Net(model.Model):
    def __init__(
        self, img_ch=3, output_ch=1, lr: float = 1e-5, optimizer_fn=torch.optim.Adam, **kwargs
    ):
    ...
    def forward(self, batch: batch.Batch) -> Tensor:
        ...
        if src.shape[1] % padd_multiple:
            ...
            src = torch.cat(
                (src, torch.zeros((src.shape[0], pad_len), device=self.device, dtype=torch.long)),
                dim=-1,
            )
        ...
        return {"structure": structure[:, : src.shape[1] - pad_len, : src.shape[1] - pad_len]}
