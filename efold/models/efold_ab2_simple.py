...
class eFold(Model):
    def __init__(
        ...
        **kwargs,
        ...
        # Freeze unused pairwise modules to prevent DDP error.
        # As the ablation's intent is to bypass the pairwise trunk.
        for block in self.eFold.blocks:
            # bias is set to None, so this is unused.
            for p in block.pair_to_sequence.parameters():
                p.requires_grad_(False)
            # These are commented out in the forward pass.
            for p in block.resNet.parameters():
                p.requires_grad_(False)
            for p in block.mlp_pair.parameters():
                p.requires_grad_(False)
...
class EvoBlock(nn.Module):
    ...
    def forward(self, sequence_state, pairwise_state):
        ...
        #Ablation 2 
        # The `pairwise_state * 0` term is numerically a no-op, but it
        # ensures that the gradient history of the pairwise state is
        # preserved, making all parameters "used" in DDP's eyes.
        pairwise_state = (pairwise_state * 0) + self.sequence_to_pair(sequence_state)
